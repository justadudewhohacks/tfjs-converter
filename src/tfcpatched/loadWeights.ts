...
export async function loadWeights(
    manifest: tfc.io.WeightsManifestConfig, filePathPrefix = '',
    weightNames?: string[],
    requestOptions?: RequestInit): Promise<tfc.NamedTensorMap> {
  ...
  const groupWeightsToFetch: {
    [group: number]: Array<{
      manifestEntry: tfc.io.WeightsManifestEntry; groupOffset: number;
      sizeBytes: number;
    }>
  } = {};
  ...
  manifest.forEach((manifestGroupConfig, groupIndex) => {
    ...
    manifestGroupConfig.weights.forEach(weightsEntry => {
      ...
      const weightsBytes = DTYPE_VALUE_SIZE_MAP[rawDtype] *
        tfc.util.sizeFromShape(weightsEntry.shape);
  ...
  const weightsTensorMap: tfc.NamedTensorMap = {};
  ...
  groupIndicesToFetch.forEach(i => {
    ...
    weightsEntries.forEach(weightsEntry => {
      ...
      weightsTensorMap[weightName] = tfc.tensor(
